...
function getmax(A,N, line,col)
	if line==N
		return getvalue(A,line,col)
	else
		# left
		a = getmax(A,N, line+1, col)
		b = getmax(A,N, line+1, col+1)
		return getvalue(A,line,col) + max(a,b)
	end
end
...
function main()
	...
	N = 15 #lines
	assert( sum(1:N) == length(A))

	println(getmax(A,N,1,1))
end
